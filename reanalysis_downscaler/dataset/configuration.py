...
from reanalysis_downscaler.dataset.files import DataFile, DataFileCollection

DATA_SPLIT_NAMES = ["train", "validation", "test"]


class DataConfiguration:
    def __init__(self, data_configuration: Union[str, Path, Dict]):
        """
        ...
        """
        if isinstance(data_configuration, str) or isinstance(data_configuration, Path):
            with open(data_configuration) as file:
                data_configuration = yaml.safe_load(file)
